...
for(idir in 1:length(dirs)){
  ...
  # annual mean
  klima_df$year<- lubridate::year(klima_df$Date)

  run_id<-stringr::str_sub(dirs[idir], -6)

 klima_df<-klima_df%>%
   dplyr::select(-id)%>%
   dplyr::group_by(year, corners)%>%
    dplyr::summarise_all(mean, na.rm=T)

  klima_df<-klima_df%>%
    dplyr::mutate(Date = lubridate::ymd(year, truncated = 2L))

  clima_plot_cornors(data=klima_df,
                     run_id = run_id,
